...
function createTable(connection) {
    ...
    try {
        ...
        const queryCreate = "CREATE TABLE IF NOT EXISTS people (id int auto_increment NOT NULL, name VARCHAR(255), primary key(id))"
...
function buildList(results) {
    ...
    const tableRows = results.map(person =>`<tr><td>${person.id}</td><td>${person.name}</td></tr>`).join('')
